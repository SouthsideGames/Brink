...
namespace Brink.UI
{
    ...
    public class TerminalShellController : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            contentHost.RegisterCallback<GeometryChangedEvent>(_ => UpdateMetrics());
        ...
        void UpdateStatusBar() { var gc = GameController.Instance; if (crisisIndicator != null && gc.AwaitingAssessment) crisisIndicator.style.display = DisplayStyle.None; if (!gc.IsRunning) { classificationLabel.text = BannerText(); return; } dateLabel.text = gc.State.date.DisplayString; bool spent = gc.State.CommandCapacitySpent; bool roomy = sizeClass == SizeClass.Large; cpLabel.text = spent && roomy ? $"CP 0 — MONTH SPENT   INF {gc.State.influence}  PC {gc.State.politicalCapital:F0}" : $"CP {gc.State.commandPoints.current}  INF {gc.State.influence}  PC {gc.State.politicalCapital:F0}"; cpLabel.EnableInClassList("status-stat-spent", spent); crisisIndicator.style.display = !gc.AwaitingAssessment && gc.State.HasOpenCrisis ? DisplayStyle.Flex : DisplayStyle.None; UpdateAttention(gc.State); }
        ...
        void UpdateCrisisOverlay() { var gc = GameController.Instance; var crises = gc.IsRunning ? gc.State.activeCrises : null; if (crisisOverlay == null) return; bool active = !gc.AwaitingAssessment && crises != null && crises.Count > 0; crisisOverlay.style.display = active ? DisplayStyle.Flex : DisplayStyle.None; if (!active) { crisisPanel?.Hide(); return; } var crisis = crises[0]; crisisPanel?.Show(crisis, index => { GameController.Instance.ResolveCrisis(crisis, index); RefreshAll(); }); ApplyTextPolicy(crisisOverlay, DisplaySettings.ParagraphSpacing, TerminalMetrics.OverlayColumns); }
