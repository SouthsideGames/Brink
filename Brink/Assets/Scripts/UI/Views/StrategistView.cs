...
namespace Brink.UI.Views
{
    public class StrategistView : TerminalView
    {
        ...
        void BuildStrategy(GameState state)
        {
            ...
            if (plan != null) foreach (var objective in plan.objectives) { var captured = objective; var row = new VisualElement(); row.AddToClassList("button-row"); Root.Add(row); if (captured.condition == null) { var status = new Button(() => { StrategySystem.SetFreeformObjectiveMet(state, captured.id, !captured.achieved); Refresh(); }) { text = captured.achieved ? "REOPEN" : "MARK MET" }; status.AddToClassList("cmd-button"); row.Add(status); } var remove = new Button(() => { StrategySystem.RemoveObjective(state, captured.id); Refresh(); }) { text = "REMOVE — " + captured.title }; remove.AddToClassList("cmd-button"); row.Add(remove); }
        }
