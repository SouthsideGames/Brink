...
namespace Brink.UI.Views
{
    public class StrategistView : TerminalView
    {
        ...
        void BuildStrategy(GameState state)
        {
            ...
            AddText("terminal-text-dim").text = " WRITE AN OBJECTIVE — measured condition or self-assessed freeform intent; no XP/grade bonus, maximum three.";
            var title = new TextField("OBJECTIVE") { value = "" }; title.AddToClassList("terminal-input"); Root.Add(title);
            var kinds = new List<string> { "Stability", "Approval", "Unity", "Energy", "Food", "Materials", "Industry", "Military", "Economy", "Intelligence", "Diplomacy", "Government", "Treaties", "Capabilities", "Relations ≥", "Relations ≤", "Solvent", "Freeform" };
            ...
            var add = new Button(() => { if (plan != null && plan.objectives.Count >= StrategySystem.MaxObjectives) { feedback.text = " OBJECTIVE LIMIT REACHED — remove one of the three standing objectives first."; return; } if (kind.value == "Freeform") { if (!StrategySystem.AddFreeformObjective(state, title.value)) { feedback.text = " OBJECTIVE REJECTED — freeform intent needs a title."; return; } Refresh(); return; } var condition = ObjectiveCondition(kind.value, threshold.value, param.value); if (condition == null) { feedback.text = " OBJECTIVE REJECTED — relationship measures require a country ID."; return; } if (!StrategySystem.AddObjective(state, string.IsNullOrWhiteSpace(title.value) ? condition.text : title.value, condition)) { feedback.text = " OBJECTIVE REJECTED — this measure cannot be used as a self-authored standing goal."; return; } Refresh(); }) { text = "ADD OBJECTIVE" }; add.AddToClassList("cmd-button"); add.AddToClassList("primary"); Root.Add(add);
            if (plan != null) foreach (var objective in plan.objectives) { var captured = objective; var row = new VisualElement(); row.AddToClassList("button-row"); Root.Add(row); if (captured.freeform) { var status = new Button(() => { StrategySystem.SetFreeformObjectiveMet(state, captured.id, !captured.achieved); Refresh(); }) { text = captured.achieved ? "REOPEN" : "MARK MET" }; status.AddToClassList("cmd-button"); row.Add(status); } else if (!captured.achieved && StrategyCabinetBridge.TryProgrammeInstruction(captured, out _, out _)) { bool active = plan.programmeObjectiveId == captured.id; var programme = new Button(() => { if (active) StrategySystem.CancelProgramme(state); else StrategySystem.SetProgramme(state, captured.id); Refresh(); }) { text = active ? "CANCEL PROGRAMME" : "AUTHORIZE PROGRAMME" + (plan.programmeChosen ? $" [{StrategySystem.ProgrammeRevisionInfluence} INF]" : "") }; programme.AddToClassList("cmd-button"); if (active) programme.AddToClassList("primary"); row.Add(programme); } var remove = new Button(() => { StrategySystem.RemoveObjective(state, captured.id); Refresh(); }) { text = "REMOVE — " + captured.title }; remove.AddToClassList("cmd-button"); row.Add(remove); }
        }
