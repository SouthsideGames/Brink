...
namespace Brink.Tests
{
    public class IntelligenceSystemTests
    {
        ...
        // ---------- the mole hunt (spec 03 §7c) ----------

        [Test]
        public void AHuntThatFindsNothingStillCostsUs()
        {
            // **The design.** A free scan would be strictly correct to run every
            // month, which is not a decision. Suspicion turned inward has to cost
            // something whether or not it was warranted.
            var player = state.PlayerCountry;
            state.networks.Clear();               // nobody is inside us
            float cohesion = player.government.eliteCohesion;

            Assert.IsFalse(IntelligenceSystem.MoleHuntBy(state, state.playerCountryId),
                "A hunt found a foreign service in a world containing none.");
            Assert.Less(player.government.eliteCohesion, cohesion,
                "Searching our own people for a spy who was not there cost nothing, so hunting "
                + "every month would be free and correct.");
        }

        [Test]
        public void AHuntCanFindARealPenetration()
        {
            var player = state.PlayerCountry;
            player.counterIntel.counterIntelligence = 95f;

            var hostile = new IntelNetwork
            {
                ownerId = "CHN",
            ...
            state.networks.Add(hostile);

            // Deterministic per action, so run a handful rather than trusting one
            // draw to decide whether the mechanism exists at all.
            bool found = false;
            for (int attempt = 0; attempt < 8 && !found; attempt++)
                found = IntelligenceSystem.MoleHuntBy(state, state.playerCountryId);

            Assert.IsTrue(found,
                "A capable service hunting eight times never found a 90-penetration network "
                + "sitting inside it.");
            Assert.IsTrue(hostile.compromised, "It was found and left in place.");
        }

        [Test]
        public void BeingCaughtByAHuntGoesOnThePublicRecord()
        {
            // The counter-play chain runs on the public record — and this session
            // found two separate places where a "we were caught" line never
            // reached it. A new catch path must not be the third.
            var player = state.PlayerCountry;
            player.counterIntel.counterIntelligence = 99f;
            ...
            for (int attempt = 0; attempt < 8; attempt++)
                if (IntelligenceSystem.MoleHuntBy(state, state.playerCountryId)) break;

            bool recorded = false;
            foreach (var entry in state.chronicle)
                if (IntelligenceSystem.IsPublicSubversionRecord(entry, "CHN")) recorded = true;

            Assert.IsTrue(recorded,
                "A network rolled up by a mole hunt taught the world nothing, because the catch "
                + "never reached the public record the expectation layer reads.");
        }

        // ---------- diminishing returns on working a network (spec 03 §6a) ----------

        [Test]
        public void RepeatedOperationsAgainstTheSameStateGetHarder()
        {
            // A deep network used to be an unlimited supply of sabotage: nothing
            // about the tenth operation differed from the first except whatever
            // had been caught in between. Tempo is what a target's services
            // notice even when they attribute nothing, so it applies to the
            // careful operator as well as the careless one — which is what makes
            // it distinct from `DirectedHardening`.
            ...
            float before = network.operationTempo;
            for (int i = 0; i < 3; i++)
                IntelligenceSystem.RunCovertOperation(state, turns, "CHN",
                    CovertOperation.Sabotage);

            Assert.Greater(network.operationTempo, before,
                "Three operations in a row drew no attention at all, so nothing about a "
                + "network gets harder the harder it is worked.");
        }

        [Test]
        public void OperationalAttentionFadesWithPatience()
        {
            // The other half, and the one this codebase gets wrong most often: a
            // penalty with no recovery path is a disqualification rather than a
            // price. The answer to a burned-out network has to be waiting.
            var network = new IntelNetwork
            {
                ownerId = state.playerCountryId,
                targetId = "CHN",
            ...
            Assert.Less(network.operationTempo, 20f,
                $"Two years of quiet left operational attention at {network.operationTempo:F1}. "
                + "A network worked hard once would never be usable again.");
        }
