...
Amendment, September 2026 (#25 port dependencies): bilateral trade with two present authored port endpoints now reads mine disruption from that physical pair, not all ground held by its partners. This is a named dependency, not an ocean itinerary or third-party chokepoint path. Unmodelled links retain the disclosed national-holder abstraction, with no double charge. Existing own-ground escort authority is unchanged. Dependencies are derived rather than saved, so content updates can change active-mine exposure in old saves without adding missing ports. See spec 02; alternate routes and wider transport geography remain pending.
...
19. Military Pillar
Amendment, September 2026: mine warfare leaves a temporary hazard on selected ground, not a permanent decrement to trade agreements. Routine clearance gives every state a recovery path, including after peace; successful convoy escort can shorten the hazard locally. Mapped trade depends on named physical ports and a bounded authored passage table; unmapped trade retains the explicit national-holder fallback. The reduction never stacks or claims complete closure. A foreign passage can therefore affect our deliveries, but being its customer grants no right to clear it. Missing passages remain absent, and content updates can change old-save exposure because dependencies are derived rather than saved. No automatic alternate routing is promised. Initial duration and clearance values require native balance measurement. See specs 01 and 02.
