...
efficiency = 50, or 50 + delivered / (treasurySpent/1000) × 6 when spent > 100, clamped 0..100
             (delivered = pillarDelta × 2.5 + gdpGrowth% × 1.5)

score = trajectory×0.18 + economy×0.20 + stability×0.14
      + position×0.10 + crisis×0.11 + initiative×0.19 + efficiency×0.08
...
was tens of thousands in the red and graded B. A deficit now costs the economy
component up to 20 points, scaled in years of income
(`EconomySystem.TreasuryIncomeRate`). Guarded by
`BugRegressionTests.Evaluation_ADeepDeficitCostsTheEconomyGrade`.
